...
class Json_To_CSV_Trivy(Json_To_CSV):
    ...
    def image_vuln_info(i):
        ...
        if 'Results' in i:
            ...
            print(i['ArtifactName'])  # this image had no results/ vulns so enter NA
            df_image.loc[0] = ['NA', 'NA', 'NA']

        return df_image

    @staticmethod
    def image_vuln_info2(i):
        df_image = pd.DataFrame(columns=['vuln_id', 'severity', 'count'])
        df_related = pd.DataFrame(columns=['vuln_id', 'severity', 'count'])

        if 'Results' in i:  # if no results then no vulns write NA

            if "Vulnerabilities" in i['Results'][0]:
                r = i['Results'][0]  # results get split into two categories.
            ...
            if "Vulnerabilities" in i['Results'][1]:
                r = i['Results'][1]  # results get split into two categories.

                for v in r['Vulnerabilities']:  # go through each vuln found in each image

                    if v['VulnerabilityID'] in df_image.values:  # if we already found this vuln in this image, just update the count

                        index_vulnId = df_related[df_related['vuln_id'] == v['VulnerabilityID']].index
                        current_vuln_count = df_related.loc[index_vulnId]['count'].values[0]
                        df_related.loc[index_vulnId] = [v['VulnerabilityID'], v['Severity'], current_vuln_count + 1]

                        if v['Severity'] != df_related.loc[index_vulnId]['severity'].values[0]:
                            print("wtf")  # a check for later if this ever happens got to go back and change this statement
                    else:
                        # making a new row of our data frame with vuln id, severity and the total count of times it was found in this image
                        new_row = [v['VulnerabilityID'], v['Severity'], int(1)]
                        df_related.loc[len(df_image.index)] = new_row
            else:
                pass
