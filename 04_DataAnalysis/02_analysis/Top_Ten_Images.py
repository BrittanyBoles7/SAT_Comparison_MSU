...
def plot_stacked_bar(data, g, t):
    ...
    plt.bar(index - bar_width / 2 - gap, unknown_g, bottom=np.add(np.add(np.add(low_g, medium_g), high_g), critical_g),
            width=bar_width, color='#9467bd', label='Unknown')
    ...
    plt.bar(index + bar_width / 2 + gap, critical, bottom=np.add(np.add(low, medium), high), width=bar_width, color='#d62728')
    plt.bar(index + bar_width / 2 + gap, unknown, bottom=np.add(np.add(np.add(low, medium), high), critical), width=bar_width, color='#9467bd')
    ...
    plt.legend(["Low", "Medium", "High", "Critical", "Unknown"], fontsize=18, loc='upper left')
